...
class DetectionValidator(BaseValidator):
    ...
    def finalize_metrics(self) -> None:
        ...
        mp, mr, map50, map95 = self.metrics.mean_results()
        ...
        rows = [
            {
                "Class": "all",
                "Images": self.seen,
                "Instances": int(self.metrics.nt_per_class.sum()),
                "Box-P": round(float(mp), decimals),
                "Box-R": round(float(mr), decimals),
                "Box-F1": round(float(f1), decimals),
                "mAP50": round(float(map50), decimals),
                "mAP50-95": round(float(map95), decimals),
            }
        ]

        rows.extend(self.metrics.summary(normalize=True, decimals=decimals))
